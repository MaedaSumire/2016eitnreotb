...
ExtraStageLookUp::ExtraStageLookUp(DeviceInterface* deviceinterface){
...
void ExtraStageLookUp::ExtraRun(){
	...
	//デバイスの初期値を取ってくる
	dv0 = m_pDeviceValueGet->DeviceValueGetter();
	dv = dv0;
	...
	while(1){
		...
		m_pMotorDrive->TailMotorDrive(tailAngle);
		m_pMotorDrive->LRMotorDrive(lmotor,rmotor);
		count++;

...
void main_task(intptr_t unused) {
	...
	char cBuff[1024];
	sprintf(cBuff, "バランス制御実行周期:0.00475000000F");
	...
	while (1) {
		...
		if (sonar_alert() == 1) /* 障害物検知 */
		{
			...
			int	nBri = 22	;
        	if( nBri-5 < g_unBrightness && g_unBrightness < nBri+5 ){
        		turn	= 0;
        	}else{
        		if( g_unBrightness > nBri + 10 ){
        			forward -= 10; /* 前進命令 */
                	turn -=  5; /* 右旋回命令 */
        		}else if( g_unBrightness > nBri + 5 ){
        			forward -= 5; /* 前進命令 */
                	turn -=  2; /* 右旋回命令 */
  				}else if( g_unBrightness < nBri - 10 ){
  					forward -= 10; /* 前進命令 */
                	turn +=  5; /* 左旋回命令 */
            	}else{
            		forward -= 5; /* 前進命令 */
		...
		// ログ
		char cBuff[1024];
		sprintf(cBuff, "Main,%d,%d,%d\n", clock->now(), g_motor_ang_r, g_motor_ang_l);
		fputs(cBuff, bt); // エコーバック

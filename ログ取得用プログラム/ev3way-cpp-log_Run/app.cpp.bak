...
void main_task(intptr_t unused)
{
	...
	//調整機能を導入
    	...
    	{
    		tail_angle_stand_up = tail_angle_stand_up + 0.5;
    	}
 ...
 //       	
            ...
            //if (unAmbient >= (LIGHT_WHITE + LIGHT_BLACK)/2)
        	int	nAmb = 22	;
        	if( nAmb-5 < unAmbient && unAmbient < nAmb+5 ){
        		turn	= 0;
        	}else{
        		if( unAmbient > nAmb + 10 ){
        			forward -= 10; /* 前進命令 */
                	turn -=  5; /* 右旋回命令 */
        		}else if( unAmbient > nAmb + 5 ){
      				forward -= 5; /* 前進命令 */         			
                	turn -=  2; /* 右旋回命令 */
  				}else if( unAmbient < nAmb - 10 ){
        			forward -= 10; /* 前進命令 */ 					
                	turn +=  5; /* 左旋回命令 */
            	}else{
       				forward -= 5; /* 前進命令 */            		
...
*/
	sprintf( cBuff,"LOG,%d,%d\n",
				unNow, g_gyro,unAmbient,nBrightness);

...
void main_task(intptr_t unused) {
	...
	//ローカル変数の宣言
	int32_t motor_ang_l, motor_ang_r, gyro, volt, nAngle;
	uint8_t unAmbient, nBrightness;
	...
	while (1) {
		...
		unAmbient = colorSensor->getBrightness();
		nAngle = gyroSensor->getAngle();
		nBrightness = colorSensor->getBrightness();

		//tail_control(TAIL_ANGLE_DRIVE); /* バランス走行用角度に制御 */

		/* 倒立振子制御APIを呼び出し、倒立走行するための */
		/* 左右モータ出力値を得る */
		balance_control((float) forward, (float) turn, (float) gyro,
				(float) GYRO_OFFSET, (float) motor_ang_l, (float) motor_ang_r,
				(float) volt, (int8_t *) &pwm_L, (int8_t *) &pwm_R);
		...
		if (sonar_alert() == 1) /* 障害物検知 */
		{
			...
			forward = 30; /* 前進命令 */
			if (unAmbient >= (LIGHT_WHITE + LIGHT_BLACK) / 2) {
				turn = 20; /* 左旋回命令 */
			} else {
				turn = -20; /* 右旋回命令 */
			}
